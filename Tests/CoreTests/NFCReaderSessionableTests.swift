...
final class NFCReaderSessionableTests: XCTestCase {
    func testNFCReaderSessionableInheritedByNFCNDEFReaderSession() throws {
        #if canImport(CoreNFC)
    ...
    func testNFCReaderSessionableInheritedByNFCTagReaderSession() throws {
        #if canImport(CoreNFC)
        XCTAssertIdentical(NFCTagReaderSession.Session.self, NFCTagReaderSession.self)
        // XCTAssertIdentical(NFCTagReaderSession.Delegate.self as AnyObject, NFCTagReaderSessionDelegate.self as AnyObject) // TODO: wait fixing https://gist.github.com/treastrain/6e9358d3e715720568004d99aabb25fc
        #else
        throw XCTSkip("Support for this platform is not considered.")
        #endif
    }
    
    func testNFCReaderSessionableInheritedByNFCVASReaderSession() throws {
        #if canImport(CoreNFC)
